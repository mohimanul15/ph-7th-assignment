...
const SelectedSingle = ({single,playerDelete}) => {
    
    return (
        <div className="w-full px-4 border-2 py-2 border-main-10 rounded-xl">
            <div className="flex justify-between items-center">
                
                <div className="flex gap-4 items-center">
                    <div>
                        <img src={single.image} className="w-24 h-auto rounded-xl"/>
                    </div>

                    <div className="text-start">
                        <h3 className="font-semibold text-2xl text-main-100">{single.name}</h3>
                        <p className="font-normal text-base text-main-70">{single.battingType}</p>
                    </div>
                </div>

                <div>
                    <button onClick={()=>playerDelete(single.playerId)}>
